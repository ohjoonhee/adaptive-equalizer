...
class Denoiser(nn.Module):
    ...
    def denoise_multiple(self, noisy_samples):
        ...
        noisy_samples = torch.stack(
            [F.pad(noisy, (0, max_len - noisy.shape[-1])) for noisy in noisy_samples]
        )
        denoised_samples = self.denoise_single(noisy_samples)

        return [
            denoised[..., :audio_len]
            for (denoised, audio_len) in zip(denoised_samples, audio_lens)
        ]
    ...
    def denoise(self, noisy_dir, denoised_dir=None):
        ...
        noisy_samples = [
            torch.tensor(librosa.load(wav_file, sr=16000)[0])
        ...
        if denoised_dir is not None:
            ...
            for denoised, noisy_fn in zip(denoised_samples, noisy_files):
                torchaudio.save(
                    denoised_dir / f"{noisy_fn.stem}.wav", denoised[None, :], 16000
                )
